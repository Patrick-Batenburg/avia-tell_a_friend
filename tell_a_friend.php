...
if ( !class_exists( 'avia_sc_tell_a_friend' ) )
{
	class avia_sc_tell_a_friend extends aviaShortcodeTemplate
	{
		/**
		...
		*/
		...
		{
			$this->config['name']		= __('Tell a Friend', 'avia_framework' );
			$this->config['tab']		= __('Content Elements', 'avia_framework' );
			$this->config['icon']		= AviaBuilder::$path['imagesURL']."sc-contact.png";
			$this->config['order']		= 0;
			$this->config['target']		= 'avia-target-insert';
			$this->config['shortcode'] 	= 'av_tell_a_friend';
			$this->config['shortcode_nested'] = array('av_tell_a_friend_field');
			$this->config['tooltip'] 	= __('Creates a customizable tell a friend form', 'avia_framework' );
		}
		...
		*/
		function popup_elements()
		{
			$this->elements = apply_filters(
				'avf_sc_contact_popup_elements', array(
					array(
					...
					array(
						...
						"id" 		=> "title",
						"std"  		=> __("Tell a Friend", 'avia_framework'),
						"type" 		=> "input"
					),
					...
					array(
						...
						"desc" 			=> 
							__("Here you can edit the form Elements of your tell a friend form.", 'avia_framework') . "<br/>" . 
							__("Available form elements are: single line Input elements and Textareas.", 'avia_framework') . "<br/><br/>",	
						"type" 			=> "modal_group",
						"id" 			=> "content",
					...
					array(
						...
						"desc" 		=>
							__("Enter a message that will be sent to the users email address once he has submitted the form.", 'avia_framework') . "<br/><br/>" .
					...
					array(
						...
						"subtype" 	=> array(
							...
							__('Display Captcha', 'avia_framework') 		=>'active'
						)
					),
		...
		*/
		function shortcode_handler($atts, $content = "", $shortcodename = "", $meta = "")
		{
			$atts = shortcode_atts(
				apply_filters(
					'avf_sc_tell_a_friend_default_atts', 
					array(
						...
						'title'			=> __("Tell a Friend", 'avia_framework'),
						'color'			=> "",
						'hide_labels'	=> "",
						'form_align'	=> ""
					)
			...
			// Merge all fields
			$form_fields = apply_filters('avia_tell_a_friend_form_elements', array_merge($form_fields, $elements));
			$form_fields = apply_filters('avf_sc_tell_a_friend_form_elements', $form_fields, $atts);
			$form_args = apply_filters('avia_tell_a_friend_form_args', $form_args, $post_id);
